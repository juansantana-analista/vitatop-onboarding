...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'register') {
        ...
        try {
            ...
            if ($response['status'] === 'success') {
                ...
                // IMPORTANTE: Extrair pessoa_id e endereco_id da resposta da API
                if (isset($response['data']['data'])) {
                    $apiData = $response['data']['data'];
                    
                    // Armazenar os IDs que serão necessários para o pagamento
                    $responseData['data'] = [
                        'pessoa_id' => $apiData['pessoa_id'] ?? null,
                        'endereco_id' => $apiData['endereco_id'] ?? null
                    ];
                    
                    // Armazenar também na sessão como backup
                    $_SESSION['pessoa_id'] = $apiData['pessoa_id'] ?? null;
                    $_SESSION['endereco_id'] = $apiData['endereco_id'] ?? null;
                }
    ...
    // Check if this is a payment processing request
        ...
        try {
            ...
            // Validate pessoa_id (obrigatório para pagamento)
            $pessoaId = $_POST['pessoaId'] ?? $_SESSION['pessoa_id'] ?? null;
            $enderecoId = $_POST['enderecoId'] ?? $_SESSION['endereco_id'] ?? null;
            
            if (!$pessoaId) {
                echo json_encode([
                    'status' => 'error',
                    'message' => 'ID da pessoa não encontrado. Refaça o cadastro.',
            ...
            $paymentData = [
                ...
                'valor' => $valor,
                'email' => $_SESSION['email'] ?? '' // Para identificação adicional
            ];
